...
if __name__ == "__main__":
    ...
    try:
        cnt = exec_sql("SELECT COUNT(*) AS CNT FROM PRODUTOS")
        total = int(cnt.iloc[0]["CNT"])
        print(f"📦 PRODUTOS: {total} linhas")
    except Exception as e:
        print(f"⚠️ Não foi possível contar linhas (seguindo mesmo assim): {e}")
        total = None

    # 2) Dump completo
    sql_full = "SELECT * FROM PRODUTOS"
    try:
    ...
    # 3) Salvar arquivos
    csv_path = os.path.join(OUTPUT_DIR, "PRODUTOS_FULL.csv")
    xlsx_sample_path = os.path.join(OUTPUT_DIR, "PRODUTOS_sample.xlsx")
