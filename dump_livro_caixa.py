...
if __name__ == "__main__":
    ...
    try:
        cnt = exec_sql("SELECT COUNT(*) AS CNT FROM LIVRO_CAIXA")
        total = int(cnt.iloc[0]["CNT"])
        print(f"📦 LIVRO_CAIXA: {total} linhas")
    except Exception as e:
        print(f"⚠️ Não foi possível contar linhas (seguindo mesmo assim): {e}")
        total = None

    # 2) Dump completo
    sql_full = "SELECT * FROM LIVRO_CAIXA"
    try:
        df = exec_sql(sql_full)
    except Exception as e:
        print(f"❌ Erro ao ler LIVRO_CAIXA: {e}")
        raise

    # 3) Salvar arquivos
    csv_path = os.path.join(OUTPUT_DIR, "LIVRO_CAIXA_FULL.csv")
    xlsx_sample_path = os.path.join(OUTPUT_DIR, "LIVRO_CAIXA_sample.xlsx")
