...
# ordem de tentativa de charsets
CHARSETS = []
...
if __name__ == "__main__":
    ...
    try:
        cnt = exec_sql("SELECT COUNT(*) AS CNT FROM PEDITEM")
        total = int(cnt.iloc[0]["CNT"])
        print(f"📦 PEDITEM: {total} linhas")
    except Exception as e:
        print(f"⚠️ Não consegui contar linhas (seguindo mesmo assim): {e}")

    # 2) Dump completo
    try:
        df = exec_sql("SELECT * FROM PEDITEM")
    except Exception as e:
        print(f"❌ Erro ao ler PEDITEM: {e}")
        raise

    # 3) Salvar arquivos
    csv_path = os.path.join(OUTPUT_DIR, "PEDITEM_FULL.csv")
    xlsx_sample_path = os.path.join(OUTPUT_DIR, "PEDITEM_sample.xlsx")

    try:
        df.to_csv(csv_path, index=False, encoding="utf-8-sig")
        print(f"✅ CSV completo -> {csv_path}")
    except PermissionError:
        print(f"❌ Permissão negada ao salvar {csv_path}. Feche o arquivo e rode de novo.")
    except Exception as e:
