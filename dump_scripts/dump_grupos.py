...
def main():
    inicio = datetime.now()
    print("=" * 60)
    print("🔧 Iniciando dump de GRUPOS")
    print("=" * 60)
    
    sql_full = "SELECT * FROM GRUPOS"
    if LIMIT_ROWS:
        sql_full = f"SELECT FIRST {LIMIT_ROWS} * FROM GRUPOS"
        print(f"⚠️ MODO TESTE: Limitado a {LIMIT_ROWS} linhas")
    
    try:
        print("\n📊 Lendo dados da tabela GRUPOS...")
        ...
        print(f"\n❌ ERRO FATAL ao ler GRUPOS: {e}")
        sys.exit(1)
    
    xlsx_path = os.path.join(OUTPUT_DIR, "GRUPOS_FULL.xlsx")
