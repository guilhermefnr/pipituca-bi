# dump_gp_produto_fabricacao_iteis.py
import os
import sys
...
def main():
    inicio = datetime.now()
    print("=" * 60)
    print("📦 Iniciando dump de GP_PRODUTO_FABRICACAO_ITEIS")
    print("=" * 60)
    
    sql_full = "SELECT * FROM GP_PRODUTO_FABRICACAO_ITEIS"
    if LIMIT_ROWS:
        sql_full = f"SELECT FIRST {LIMIT_ROWS} * FROM GP_PRODUTO_FABRICACAO_ITEIS"
        print(f"⚠️ MODO TESTE: Limitado a {LIMIT_ROWS} linhas")
    
    try:
        print("\n📊 Lendo dados da tabela GP_PRODUTO_FABRICACAO_ITEIS...")
        ...
        print(f"\n❌ ERRO FATAL ao ler GP_PRODUTO_FABRICACAO_ITEIS: {e}")
        sys.exit(1)
    
    xlsx_path = os.path.join(OUTPUT_DIR, "GP_PRODUTO_FABRICACAO_ITEIS_FULL.xlsx")
    ...
    try:
        with pd.ExcelWriter(xlsx_path, engine="openpyxl") as writer:
            df.to_excel(writer, index=False, sheet_name="GP_PRODUTO_FABRICACAO_ITEIS")
