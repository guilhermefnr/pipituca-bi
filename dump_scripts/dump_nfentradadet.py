...
def main():
    inicio = datetime.now()
    print("=" * 60)
    print("📄 Iniciando dump de NFENTRADADET")
    print("=" * 60)
    
    sql_full = "SELECT * FROM NFENTRADADET"
    if LIMIT_ROWS:
        sql_full = f"SELECT FIRST {LIMIT_ROWS} * FROM NFENTRADADET"
        print(f"⚠️ MODO TESTE: Limitado a {LIMIT_ROWS} linhas")
    
    try:
        print("\n📊 Lendo dados da tabela NFENTRADADET...")
        ...
        print(f"\n❌ ERRO FATAL ao ler NFENTRADADET: {e}")
        sys.exit(1)
    
    xlsx_path = os.path.join(OUTPUT_DIR, "NFENTRADADET_FULL.xlsx")
    ...
    try:
        with pd.ExcelWriter(xlsx_path, engine="openpyxl") as writer:
            df.to_excel(writer, index=False, sheet_name="NFENTRADADET")
