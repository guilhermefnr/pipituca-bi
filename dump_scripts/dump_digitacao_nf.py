...
def main():
    inicio = datetime.now()
    print("=" * 60)
    print("📄 Iniciando dump de DIGITACAO_NF")
    print("=" * 60)
    
    sql_full = "SELECT * FROM DIGITACAO_NF"
    if LIMIT_ROWS:
        sql_full = f"SELECT FIRST {LIMIT_ROWS} * FROM DIGITACAO_NF"
        print(f"⚠️ MODO TESTE: Limitado a {LIMIT_ROWS} linhas")
    
    try:
        print("\n📊 Lendo dados da tabela DIGITACAO_NF...")
        ...
        print(f"\n❌ ERRO FATAL ao ler DIGITACAO_NF: {e}")
        sys.exit(1)
    
    xlsx_path = os.path.join(OUTPUT_DIR, "DIGITACAO_NF_FULL.xlsx")
    ...
    try:
        with pd.ExcelWriter(xlsx_path, engine="openpyxl") as writer:
            df.to_excel(writer, index=False, sheet_name="DIGITACAO_NF")
