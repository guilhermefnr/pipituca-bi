...
def main():
    inicio = datetime.now()
    print("=" * 60)
    print("📊 Iniciando dump de KARDEX")
    print("=" * 60)
    
    sql_full = "SELECT * FROM KARDEX"
    if LIMIT_ROWS:
        sql_full = f"SELECT FIRST {LIMIT_ROWS} * FROM KARDEX"
        print(f"⚠️ MODO TESTE: Limitado a {LIMIT_ROWS} linhas")
    
    try:
        print("\n📊 Lendo dados da tabela KARDEX...")
        ...
        print(f"\n❌ ERRO FATAL ao ler KARDEX: {e}")
        sys.exit(1)
    
    xlsx_path = os.path.join(OUTPUT_DIR, "KARDEX_FULL.xlsx")
