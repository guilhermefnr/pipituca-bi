...
if __name__ == "__main__":
    ...
    try:
        cnt = exec_sql("SELECT COUNT(*) AS CNT FROM TEF_TRANSACOES")
        total = int(cnt.iloc[0]["CNT"])
        print(f"📦 TEF_TRANSACOES: {total} linhas")
    except Exception as e:
        print(f"⚠️ Não foi possível contar linhas (seguindo mesmo assim): {e}")
        total = None

    # 2) Dump completo
    sql_full = "SELECT * FROM TEF_TRANSACOES"
    try:
        df = exec_sql(sql_full)
    except Exception as e:
        print(f"❌ Erro ao ler TEF_TRANSACOES: {e}")
        raise

    # 3) Salvar arquivos
    csv_path = os.path.join(OUTPUT_DIR, "TEF_TRANSACOES_FULL.csv")
    xlsx_sample_path = os.path.join(OUTPUT_DIR, "TEF_TRANSACOES_sample.xlsx")
