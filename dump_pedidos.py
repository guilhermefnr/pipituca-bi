...
if __name__ == "__main__":
    ...
    try:
        cnt = exec_sql("SELECT COUNT(*) AS CNT FROM PEDIDOS")
        total = int(cnt.iloc[0]["CNT"])
        print(f"📦 PEDIDOS: {total} linhas")
    except Exception as e:
        print(f"⚠️ Não foi possível contar linhas (seguindo mesmo assim): {e}")
        total = None

    # 2) Dump completo
    sql_full = "SELECT * FROM PEDIDOS"
    try:
    ...
    # 3) Salvar em Excel (toda a base)
    xlsx_path = os.path.join(OUTPUT_DIR, "PEDIDOS_FULL.xlsx")

    try:
        with pd.ExcelWriter(xlsx_path, engine="openpyxl") as w:
            df.to_excel(w, index=False, sheet_name="PEDIDOS")
        print(f"🧾 Base completa exportada -> {xlsx_path}")
    except PermissionError:
        print(f"❌ Permissão negada ao salvar {xlsx_path}. Feche o arquivo se estiver aberto e rode novamente.")
    except Exception as e:
