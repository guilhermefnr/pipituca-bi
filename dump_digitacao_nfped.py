...
if __name__ == "__main__":
    ...
    try:
        cnt = exec_sql("SELECT COUNT(*) AS CNT FROM DIGITACAO_NFPED")
        total = int(cnt.iloc[0]["CNT"])
        print(f"📦 DIGITACAO_NFPED: {total} linhas")
    except Exception as e:
        print(f"⚠️ Não foi possível contar linhas (seguindo mesmo assim): {e}")
        total = None

    # 2) Dump completo
    sql_full = "SELECT * FROM DIGITACAO_NFPED"
    try:
        df = exec_sql(sql_full)
    except Exception as e:
        print(f"❌ Erro ao ler DIGITACAO_NFPED: {e}")
        raise

    # 3) Salvar arquivos
    csv_path = os.path.join(OUTPUT_DIR, "DIGITACAO_NFPED_FULL.csv")
    xlsx_sample_path = os.path.join(OUTPUT_DIR, "DIGITACAO_NFPED_sample.xlsx")
