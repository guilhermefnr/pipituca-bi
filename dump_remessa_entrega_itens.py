...
if __name__ == "__main__":
    ...
    try:
        cnt = exec_sql("SELECT COUNT(*) AS CNT FROM REMESSA_ENTREGA_ITENS")
        total = int(cnt.iloc[0]["CNT"])
        print(f"📦 REMESSA_ENTREGA_ITENS: {total} linhas")
    except Exception as e:
        print(f"⚠️ Não foi possível contar linhas (seguindo mesmo assim): {e}")
        total = None

    # 2) Dump completo
    sql_full = "SELECT * FROM REMESSA_ENTREGA_ITENS"
    try:
        df = exec_sql(sql_full)
    except Exception as e:
        print(f"❌ Erro ao ler REMESSA_ENTREGA_ITENS: {e}")
        raise

    # 3) Salvar arquivos
    csv_path = os.path.join(OUTPUT_DIR, "REMESSA_ENTREGA_ITENS_FULL.csv")
    xlsx_sample_path = os.path.join(OUTPUT_DIR, "REMESSA_ENTREGA_ITENS_sample.xlsx")
