...
if __name__ == "__main__":
    ...
    try:
        cnt = exec_sql("SELECT COUNT(*) AS CNT FROM TRANSAC_PEDITEM")
        total = int(cnt.iloc[0]["CNT"])
        print(f"📦 TRANSAC_PEDITEM: {total} linhas")
    except Exception as e:
        print(f"⚠️ Não foi possível contar linhas (seguindo mesmo assim): {e}")
        total = None

    # 2) Dump completo
    sql_full = "SELECT * FROM TRANSAC_PEDITEM"
    try:
        df = exec_sql(sql_full)
    except Exception as e:
        print(f"❌ Erro ao ler TRANSAC_PEDITEM: {e}")
        raise

    # 3) Salvar arquivos
    csv_path = os.path.join(OUTPUT_DIR, "TRANSAC_PEDITEM_FULL.csv")
    xlsx_sample_path = os.path.join(OUTPUT_DIR, "TRANSAC_PEDITEM_sample.xlsx")
