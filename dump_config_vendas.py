...
if __name__ == "__main__":
    ...
    try:
        cnt = exec_sql("SELECT COUNT(*) AS CNT FROM CONFIG_VENDAS")
        total = int(cnt.iloc[0]["CNT"])
        print(f"📦 CONFIG_VENDAS: {total} linhas")
    except Exception as e:
        print(f"⚠️ Não foi possível contar linhas (seguindo mesmo assim): {e}")
        total = None

    # 2) Dump completo
    sql_full = "SELECT * FROM CONFIG_VENDAS"
    try:
        df = exec_sql(sql_full)
    except Exception as e:
        print(f"❌ Erro ao ler CONFIG_VENDAS: {e}")
        raise

    # 3) Salvar arquivos
    csv_path = os.path.join(OUTPUT_DIR, "CONFIG_VENDAS_FULL.csv")
    xlsx_sample_path = os.path.join(OUTPUT_DIR, "CONFIG_VENDAS_sample.xlsx")
