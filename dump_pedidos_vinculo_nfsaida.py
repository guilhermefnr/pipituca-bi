# dump_pedidos_vinculo_nfsaida.py
import os
import pandas as pd
import firebirdsql
import config
...
if __name__ == "__main__":
    ...
    try:
        cnt = exec_sql("SELECT COUNT(*) AS CNT FROM PEDIDOS_VINCULO_NFSAIDA")
        total = int(cnt.iloc[0]["CNT"])
        print(f"📦 PEDIDOS_VINCULO_NFSAIDA: {total} linhas")
    except Exception as e:
        print(f"⚠️ Não foi possível contar linhas (seguindo mesmo assim): {e}")
        total = None

    # 2) Dump completo
    sql_full = "SELECT * FROM PEDIDOS_VINCULO_NFSAIDA"
    try:
        df = exec_sql(sql_full)
    except Exception as e:
        print(f"❌ Erro ao ler PEDIDOS_VINCULO_NFSAIDA: {e}")
        raise

    # 3) Salvar arquivos
    csv_path = os.path.join(OUTPUT_DIR, "PEDIDOS_VINCULO_NFSAIDA_FULL.csv")
    xlsx_sample_path = os.path.join(OUTPUT_DIR, "PEDIDOS_VINCULO_NFSAIDA_sample.xlsx")
