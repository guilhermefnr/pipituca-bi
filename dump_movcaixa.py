...
if __name__ == "__main__":
    ...
    try:
        cnt = exec_sql("SELECT COUNT(*) AS CNT FROM MOVCAIXA")
        total = int(cnt.iloc[0]["CNT"])
        print(f"📦 MOVCAIXA: {total} linhas")
    except Exception as e:
        print(f"⚠️ Não foi possível contar linhas (seguindo mesmo assim): {e}")
        total = None

    # 2) Dump completo
    sql_full = "SELECT * FROM MOVCAIXA"
    try:
        df = exec_sql(sql_full)
    except Exception as e:
        print(f"❌ Erro ao ler MOVCAIXA: {e}")
        raise

    # 3) Salvar arquivos
    csv_path = os.path.join(OUTPUT_DIR, "MOVCAIXA_FULL.csv")
    xlsx_sample_path = os.path.join(OUTPUT_DIR, "MOVCAIXA_sample.xlsx")
