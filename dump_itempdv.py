...
if __name__ == "__main__":
    ...
    try:
        cnt = exec_sql("SELECT COUNT(*) AS CNT FROM ITEMPDV")
        total = int(cnt.iloc[0]["CNT"])
        print(f"📦 ITEMPDV: {total} linhas")
    except Exception as e:
        print(f"⚠️ Não foi possível contar linhas (seguindo mesmo assim): {e}")
        total = None

    # 2) Dump completo
    sql_full = "SELECT * FROM ITEMPDV"
    try:
        df = exec_sql(sql_full)
    except Exception as e:
        print(f"❌ Erro ao ler ITEMPDV: {e}")
        raise

    # 3) Salvar arquivos
    csv_path = os.path.join(OUTPUT_DIR, "ITEMPDV_FULL.csv")
    xlsx_sample_path = os.path.join(OUTPUT_DIR, "ITEMPDV_sample.xlsx")
