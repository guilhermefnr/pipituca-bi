# dump_clientes_produtos_lote.py
import os
import pandas as pd
import firebirdsql
import config
...
if __name__ == "__main__":
    ...
    try:
        cnt = exec_sql("SELECT COUNT(*) AS CNT FROM CLIENTES_PRODUTOS_LOTE")
        total = int(cnt.iloc[0]["CNT"])
        print(f"📦 CLIENTES_PRODUTOS_LOTE: {total} linhas")
    except Exception as e:
        print(f"⚠️ Não foi possível contar linhas (seguindo mesmo assim): {e}")
        total = None

    # 2) Dump completo
    sql_full = "SELECT * FROM CLIENTES_PRODUTOS_LOTE"
    try:
        df = exec_sql(sql_full)
    except Exception as e:
        print(f"❌ Erro ao ler CLIENTES_PRODUTOS_LOTE: {e}")
        raise

    # 3) Salvar arquivos
    csv_path = os.path.join(OUTPUT_DIR, "CLIENTES_PRODUTOS_LOTE_FULL.csv")
    xlsx_sample_path = os.path.join(OUTPUT_DIR, "CLIENTES_PRODUTOS_LOTE_sample.xlsx")
