...
if __name__ == "__main__":
    ...
    try:
        cnt = exec_sql("SELECT COUNT(*) AS CNT FROM LANC_VENDA_POSTO")
        total = int(cnt.iloc[0]["CNT"])
        print(f"📦 LANC_VENDA_POSTO: {total} linhas")
    except Exception as e:
        print(f"⚠️ Não foi possível contar linhas (seguindo mesmo assim): {e}")
        total = None

    # 2) Dump completo
    sql_full = "SELECT * FROM LANC_VENDA_POSTO"
    try:
        df = exec_sql(sql_full)
    except Exception as e:
        print(f"❌ Erro ao ler LANC_VENDA_POSTO: {e}")
        raise

    # 3) Salvar arquivos
    csv_path = os.path.join(OUTPUT_DIR, "LANC_VENDA_POSTO_FULL.csv")
    xlsx_sample_path = os.path.join(OUTPUT_DIR, "LANC_VENDA_POSTO_sample.xlsx")
