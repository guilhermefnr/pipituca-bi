...
if __name__ == "__main__":
    ...
    try:
        cnt = exec_sql("SELECT COUNT(*) AS CNT FROM PEDIDOS_EVENTOS")
        total = int(cnt.iloc[0]["CNT"])
        print(f"📦 PEDIDOS_EVENTOS: {total} linhas")
    except Exception as e:
        print(f"⚠️ Não foi possível contar linhas (seguindo mesmo assim): {e}")
        total = None

    # 2) Dump completo
    sql_full = "SELECT * FROM PEDIDOS_EVENTOS"
    try:
        df = exec_sql(sql_full)
    except Exception as e:
        print(f"❌ Erro ao ler PEDIDOS_EVENTOS: {e}")
        raise

    # 3) Salvar arquivos
    csv_path = os.path.join(OUTPUT_DIR, "PEDIDOS_EVENTOS_FULL.csv")
    xlsx_sample_path = os.path.join(OUTPUT_DIR, "PEDIDOS_EVENTOS_sample.xlsx")
